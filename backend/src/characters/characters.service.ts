...
export class CharactersService {
  ...
  private async syncDescriptionLinks(character: { id: string; campaignId: string; description?: string | null }) {
    ...
    const byType = { E: [], C: [], L: [], O: [] } as Record<'E' | 'C' | 'L' | 'O', string[]>;
    for (const t of tokens) byType[t.type].push(t.name);

    const [events, chars, locs, objs] = await Promise.all([
      byType.E.length ? this.prisma.event.findMany({ where: { campaignId: character.campaignId, title: { in: byType.E } } }) : [],
      byType.C.length ? this.prisma.character.findMany({ where: { campaignId: character.campaignId, name: { in: byType.C } } }) : [],
      byType.L.length ? this.prisma.location.findMany({ where: { campaignId: character.campaignId, name: { in: byType.L } } }) : [],
      byType.O.length ? this.prisma.objectModel.findMany({ where: { campaignId: character.campaignId, name: { in: byType.O } } }) : [],
    ]);
    ...
    if (data.length) await this.prisma.relation.createMany({ data, skipDuplicates: true });
  }

  async listByCampaign(campaignId: string, userId: string) {
    await this.assertCampaignOwnership(campaignId, userId);
    return this.prisma.character.findMany({ where: { campaignId }, orderBy: [{ name: 'asc' }] });
  }
  ...
  async update(id: string, userId: string, dto: UpdateCharacterDto) {
    ...
    if (dto.name !== undefined && !dto.name.trim()) throw new BadRequestException('Name cannot be empty');
