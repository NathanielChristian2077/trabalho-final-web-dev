...
export class PersonagensService {
    ...
    list(campanhaId: string) {
        return this.prisma.personagem.findMany({ where: { campanhaId }, orderBy: { nome: 'asc' } });
    }
    ...
    async create(campanhaId: string, userId: string, data: { nome: string; descricao?: string }) {
        ...
        return this.prisma.personagem.create({ data: { campanhaId, nome: data.nome, descricao: data.descricao } });
    }

    async remove(id: string, userId: string) {
        const p = await this.prisma.personagem.findUnique({ where: { id }, include: { campanha: true } });
