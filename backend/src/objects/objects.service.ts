...
export class ObjectsService {
  ...
  private async syncDescriptionLinks(obj: { id: string; campaignId: string; description?: string | null }) {
    ...
    const [events, chars, locs, objs] = await Promise.all([
      byType.E.length ? this.prisma.event.findMany({ where: { campaignId: obj.campaignId, title: { in: byType.E } } }) : [],
      byType.C.length ? this.prisma.character.findMany({ where: { campaignId: obj.campaignId, name: { in: byType.C } } }) : [],
      byType.L.length ? this.prisma.location.findMany({ where: { campaignId: obj.campaignId, name: { in: byType.L } } }) : [],
      byType.O.length ? this.prisma.objectModel.findMany({ where: { campaignId: obj.campaignId, name: { in: byType.O } } }) : [],
    ]);
  ...
  async listByCampaign(campaignId: string, userId: string) {
    ...
    return this.prisma.objectModel.findMany({ where: { campaignId }, orderBy: [{ name: 'asc' }] });
  }
