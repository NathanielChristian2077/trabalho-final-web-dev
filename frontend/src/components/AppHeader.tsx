        ...
        <header className='sticky top-0 z-10 border-b bg-white/60 backdrop-blur supports-[backdrop-filter]:bg-white/40 dark:bg-zinc-900/60'>
            <div className='mx-auto flex h-14 max-w-6x1 items-center justify-between px-4'>
                ...
                <nav className='flex items-center gap-3 text-sm'>
                    ...
                    {logged ? (
                        <>
                            <Link to="/dashboard" className={linkCls(loc.pathname.startsWith('/dashboard'))}>Dashboard</Link>
                            <button
                                onClick={() => {
                    ...
                    <a href='https://github.com/NathanielChristian2077/trabalho-final-web-dev.git' className='rounded px-3 py-1.5 hover:underline'>GitHub</a>
                </nav>
            </div>
        </header>
    )
